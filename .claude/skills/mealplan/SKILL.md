...
- `pnpm nutrition:optimize` — regenerates one or more entries in `optimized-variants.json`. For each (phase × day-type × swap-combo) in scope, it treats the food pool (minus the swap state's excluded foods) as a search space and returns the macro-optimal daily quantities + meal layout, with a score and delta vs. target. This is your primary tool for "does this food fit?" and "what replaces this food if it's gone?". If you see an issue with the output, DO NOT just discount it; adjust the food/template parameters and rerun. ACTUALLY LOOK AT THE OUTPUT — it changes quantities and meal composition together to hit targets.
  - `pnpm nutrition:optimize --reconcile` — **re-keys and prunes without re-optimizing.** After a template edit that changes the swap-toggle shape (removing/adding a toggle, retiring a food), the cached variant keys no longer match the canonical format `enumerateAll` now produces. Reconcile rewrites each in-scope entry under its current canonical key — inferring the swap state from the foods actually in its meals — drops entries whose meals reference a now-unavailable food (e.g. `maxServingAmountPerPlan: 0`) or that collide on a claimed key, and reports any canonical keys left `Missing` (those still need a real `nutrition:optimize` run). Honors `--phase` / `--day` scoping (defaults to all pairs); ignores `--variant-id`. Use this instead of regenerating when the existing meals are still valid and you only need the keys cleaned up. It preserves meals verbatim — it does **not** re-solve macros.
